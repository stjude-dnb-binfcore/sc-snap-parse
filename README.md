...
Metadata files are **tab-separated (TSV)**. Each row is one sample or sublibrary. The `ID`, `SMR_ID`, or `sublibrary_ID` columns for any metadata file must contain **unique** values.
