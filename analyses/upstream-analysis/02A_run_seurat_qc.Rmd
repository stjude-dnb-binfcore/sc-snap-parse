---
title: "Seurat QC for sc-/sn-RNA-Seq Analysis in PARSE BIOSCIENCES sequencing technology data"
...
sublibrary_samples_plots_dir <- 
  file.path(seurat_qc_plots_dir, paste0(sample_name[i], "/sublibrary"))
if (!dir.exists(sublibrary_samples_plots_dir)) {
  dir.create(sublibrary_samples_plots_dir)
}
...
source(paste0(analysis_dir, "/util/import_updated_parse.R"))
source(paste0(analysis_dir, "/util/generate_QC_Plots_sublibrary_ID.R"))
```
...
# Read in PARSE BIOSCIENCES sequencing technology data

We will first read read counts in the PARSE BIOSCIENCES sequencing technology data. There is the option to use the count or the corrected matrix as generated by [SoupX](https://cran.r-project.org/web/packages/SoupX/vignettes/pbmcTutorial.html). This is an optional step, so it will be skipped or not as defined in `params`. 
...
if (use_SoupX_filtering == "YES"){
  print_message <- "we will use SoupX corrected matrix"
   ...
   # we will update the name of the assay in the `04_run_filter_object.Rmd`
    ...
    print_message <- "we will skip usage of the SoupX corrected matrix and use the count matrix"

    input_dir <- file.path(data_dir, sample_name[i], "DGE_filtered") # alignment input file dir

    # File path to data with the filtered_feature_bc_matrix
    cat(paste(c("", sprintf("`%s`", list.files(input_dir), full.names = TRUE))),
          collapse = "\n- ")
    cat("\n\n")
  
    # Load counts 
    message_str <- "\n\n ========== import counts matrix ========== \n\n"
    counts_mat <- ReadParseBio(input_dir,
                               sample_name = sample_name[i])
    
    # Rename by adding sample name in the cells
    colnames(counts_mat) <- paste0(sample_name[i], ":", colnames(counts_mat))
    
    # Read in cell metadata
    cell_metadata <- read.delim(paste0(input_dir, "/cell_metadata.csv"), sep = ",")

    # Create seurat object using gene expression data
    seurat_obj <- create_seurat_obj(counts_matrix = counts_mat,
                                    assay = "RNA",
                                    cell_metadata_info = cell_metadata) 
}
...
# Get suffix and set rownames and Add sublibrary based on suffixes
seurat_obj$sublibrary_ID <- sub(".*__([^_]+)$", "\\1", rownames(seurat_obj@meta.data))

# Let's use the ordered list with the sublibrary_ID to map s1, s2, … → our ordered list and replace accordingly.
sublib_list_dir <- dirname(data_dir)
sublib_list_file <- read.delim(file.path(sublib_list_dir, "sublib_list.txt"), sep = ",", header = FALSE, stringsAsFactors = FALSE)
# Keep original
seurat_obj$sublibrary_ID_original <- seurat_obj$sublibrary_ID

# Map new IDs
idx <- as.integer(sub("s", "", seurat_obj$sublibrary_ID_original))
seurat_obj$sublibrary_ID <- sublib_list_file$V1[idx]
# head(seurat_obj@meta.data)
seurat_obj$sublibrary_ID <- factor(seurat_obj$sublibrary_ID)
# Optional check
head(seurat_obj@meta.data[, c("sublibrary_ID_original", "sublibrary_ID")])
...
Generate_QC_Plots_1(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "02", Analysis_Name = sample_name[i], File_Name = "QC_Raw_Data", palette = gradient_palette_df)
Generate_QC_Plots_2(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "03", Analysis_Name = sample_name[i], File_Name = "QC_Raw_Data", palette = gradient_palette_df)
Generate_QC_Plots_3(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "04", Analysis_Name = sample_name[i], File_Name = "QC_Raw_Data", palette = gradient_palette_df)
Generate_QC_Plots_4(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "05", Analysis_Name = sample_name[i], File_Name = "QC_Raw_Data", palette = gradient_palette_df)
Generate_QC_Plots_5(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "06", Analysis_Name = sample_name[i], File_Name = "QC_Raw_Data", palette = gradient_palette_df)
Generate_QC_Plots_6(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "07", Analysis_Name = sample_name[i], File_Name = "QC_Raw_Data", palette = gradient_palette_df)
```

## Per Sub-library

```{r plot-data-before-filtering-sublibrary, fig.width = 14, fig.height = 8, fig.fullwidth = TRUE, echo=TRUE}
sublibrary_IDs <- unique(as.character(seurat_obj$sublibrary_ID))
sublibrary_IDs <- sort(sublibrary_IDs, decreasing = FALSE)
print(sublibrary_IDs)

for (x in sublibrary_IDs) {
  cat("Generating QC plots for RAW data Per Sub-library for sample", sample_name[i], "sub-library", x, "\n")
  
  # Create `per_sublibrary_samples_plots_dir` Per Sub-library for sample
  per_sublibrary_samples_plots_dir <- file.path(sublibrary_samples_plots_dir, x)
  if (!dir.exists(per_sublibrary_samples_plots_dir)) {
    dir.create(per_sublibrary_samples_plots_dir)}
    
  # Subset object for current sublibrary
  seurat_obj_sub <- subset(seurat_obj, subset = sublibrary_ID == x)
  
  fname <- file.path(sublibrary_samples_plots_dir, sprintf("%s", x), sprintf("%s_01_before_filter_vln_plot_sublibrary.png", x))
  p <- print(create_vln_plot(seurat_obj = seurat_obj_sub, grouping = "sublibrary_ID", min_genes = min_genes, min_count = min_count, title_name = paste("Unfiltered-data:", sample_name[i], x), palette = palette_df))
  ggsave(filename = fname, plot = p, width = 8, height = 6, device = "png")

  print(Generate_QC_Plots_1_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "02", Analysis_Name = sprintf("%s", x), File_Name = "QC_Raw_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_2_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "03", Analysis_Name = sprintf("%s", x), File_Name = "QC_Raw_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_3_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "04", Analysis_Name = sprintf("%s", x), File_Name = "QC_Raw_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_4_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "05", Analysis_Name = sprintf("%s", x), File_Name = "QC_Raw_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_5_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "06", Analysis_Name = sprintf("%s", x), File_Name = "QC_Raw_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_6_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "07", Analysis_Name = sprintf("%s", x), File_Name = "QC_Raw_Data_sublibrary", palette = gradient_palette_df))
}
```
...
if (use_QC_default == "YES"){
  ## Approach 2: Default QC (with mito MAD)
  print("We will QC with default parameters (with mito MAD) and/or the `Find_Outlier_Thershold` function.")
  seurat_obj <- run_QC_default(seurat_obj = seurat_obj)
  }

if (use_QC_default_no_mito_MAD == "YES"){
    ## Approach 3: Default QC (without mito MAD)
    print("We will QC with default parameters WITHOUT mito MAD filtering and/or the `Find_Outlier_Thershold` function.")
    seurat_obj <- run_QC_default_no_mito_MAD(seurat_obj = seurat_obj)
    }
```
...
Generate_QC_Plots_1(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "11", Analysis_Name = sample_name[i], File_Name = "QC_Filtered_Data", palette = gradient_palette_df)
Generate_QC_Plots_2(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "12", Analysis_Name = sample_name[i], File_Name = "QC_Filtered_Data", palette = gradient_palette_df)
Generate_QC_Plots_3(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "13", Analysis_Name = sample_name[i], File_Name = "QC_Filtered_Data", palette = gradient_palette_df)
Generate_QC_Plots_4(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "14", Analysis_Name = sample_name[i], File_Name = "QC_Filtered_Data", palette = gradient_palette_df)
Generate_QC_Plots_5(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "15", Analysis_Name = sample_name[i], File_Name = "QC_Filtered_Data", palette = gradient_palette_df)
Generate_QC_Plots_6(Seurat_obj = seurat_obj, Project_Path = seurat_qc_plots_dir, Figure_Total = "16", Analysis_Name = sample_name[i], File_Name = "QC_Filtered_Data", palette = gradient_palette_df)
```

## Per Sub-library

```{r plot-data-after-filter-sublibrary, fig.width = 14, fig.height = 8, fig.fullwidth = TRUE, echo=TRUE}
sublibrary_IDs <- unique(as.character(seurat_obj$sublibrary_ID))
sublibrary_IDs <- sort(sublibrary_IDs, decreasing = FALSE)
print(sublibrary_IDs)

for (x in sublibrary_IDs) {
  cat("Generating QC plots for FILTERED data Per Sub-library for sample", sample_name[i], "sub-library", x, "\n")
  ...
  fname <- file.path(sublibrary_samples_plots_dir, sprintf("%s", x), sprintf("%s_01_after_filter_vln_plot_sublibrary.png", x))
  p <- print(create_vln_plot(seurat_obj = seurat_obj_sub, grouping = "sublibrary_ID", min_genes = min_genes, min_count = min_count, title_name = paste("Filtered-data:", sample_name[i], x), palette = palette_df))
  ggsave(filename = fname, plot = p, width = 8, height = 6, device = "png")

  print(Generate_QC_Plots_1_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "08", Analysis_Name = sprintf("%s", x), File_Name = "QC_Filtered_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_2_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "09", Analysis_Name = sprintf("%s", x), File_Name = "QC_Filtered_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_3_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "10", Analysis_Name = sprintf("%s", x), File_Name = "QC_Filtered_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_4_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "11", Analysis_Name = sprintf("%s", x), File_Name = "QC_Filtered_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_5_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "12", Analysis_Name = sprintf("%s", x), File_Name = "QC_Filtered_Data_sublibrary", palette = gradient_palette_df))
  print(Generate_QC_Plots_6_sublibrary_ID(Seurat_obj = seurat_obj_sub, Project_Path = sublibrary_samples_plots_dir, Figure_Total = "13", Analysis_Name = sprintf("%s", x), File_Name = "QC_Filtered_Data_sublibrary", palette = gradient_palette_df))
}
```
