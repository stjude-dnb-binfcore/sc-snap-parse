...
We will first read read counts in the 10x data. There is the option to use the count or the corrected matrix as generated by [SoupX](https://cran.r-project.org/web/packages/SoupX/vignettes/pbmcTutorial.html). This is an optional step, so it will be skipped or not as defined in `params`. 
...
if (use_SoupX_filtering == "YES"){
  print_message <- "we will use SoupX corrected matrix"
   ...
   # we will update the name of the assay in the `04_run_filter_object.Rmd`
    ...
    cat(paste(c("", sprintf("`%s`", list.files(paste(input_dir, "/DGE_filtered"), full.names = TRUE))),
          collapse = "\n- "))
    cat("\n\n")
