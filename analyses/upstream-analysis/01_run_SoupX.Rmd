---
title: "Estimating and filtering out ambient mRNA for sc-/sn-RNA-Seq Analysis in PARSE BIOSCIENCES sequencing technology data"
author: "Antonia Chroni and Sharon Freshour for SJCRH DNB_BINF_Core"
papersize: a4
fontsize: 11pt
links-as-notes: true
output:
...
for (i in seq_along(sample_name)) {
  ...
  filtered_matrix_file <- file.path(data_dir, sample_name[i], "DGE_filtered") #"count_matrix.mtx"
  raw_matrix_file <- file.path(data_dir, sample_name[i], "DGE_unfiltered") #"count_matrix.mtx"
  clusters_file <- file.path(data_dir, sample_name[i], "report", "cluster_assignment.csv") 

  # Read data
  filtered_matrix <- ReadParseBio(filtered_matrix_file, use.names = TRUE)
  raw_matrix <- ReadParseBio(raw_matrix_file, use.names = TRUE)
  clusters <- read.csv(clusters_file)
  
  # Order clustering data to match order of columns in filtered_matrix
  rownames(clusters) <- clusters$bc_wells
  clusters <- clusters[colnames(filtered_matrix),]
  ...
  sc_raw <- SoupChannel(raw_matrix, filtered_matrix)
  
  # Explicitly set clusters for sc_raw object using clusters metadata loaded above
  sc_raw <- setClusters(sc_raw, setNames(clusters$cluster, rownames(clusters)))
