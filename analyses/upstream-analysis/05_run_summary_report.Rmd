...
3. `expected_dbr_` (`scDblFinder.class.expected_dbr`): higher fixed expected doublet rate as specified in the `yaml`. The user can specify the value to use here, e.g. based on the anticipated proportion of doublets in the dataset. The default value provided in the `yaml` is 0.035 (i.e. 3.5%).
...
We will summarize the estimated and removed doublets per sample and per method. Because expected doublet rates for Parse Biosciences libraries may vary by sample type, loading strategy, and tissue characteristics, three scDblFinder parameterizations (`no_dbr_dbr.sd_1`, `low_dbr_0.0003`, `expected_dbr_`) and three `scDblFinder_prior_sensitivity` strategies (`scDblFinder_prior_sensitivity_any`, `scDblFinder_prior_sensitivity_majority`, `scDblFinder_prior_sensitivity_all`) were evaluated per sample. Results are shown by sample (as ordered in the `Samples` section). This is an optional step, so it will be skipped or not as defined in `params`.
...
Doublet prediction UMAP plots are generated for each sample and each evaluated method (`no_dbr_dbr.sd_1`, `low_dbr_0.0003`, `expected_dbr_`, and `scDblFinder_prior_sensitivity` strategies).
...
[scDblFinder](https://bioconductor.org/packages/devel/bioc/vignettes/scDblFinder/inst/doc/scDblFinder.html) is an R package designed for identifying doublets in single-cell RNA-seq data. For Parse Biosciences data, this pipeline evaluates multiple parameterizations (`dbr.sd = 1`, `dbr = 0.0003`, `dbr = expected`) and `scDblFinder_prior_sensitivity` strategies to account for uncertain platform-specific doublet rates.
