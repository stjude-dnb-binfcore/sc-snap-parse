...
Because platform-specific expected doublet rates for Parse Biosciences data can be uncertain, this pipeline evaluates **three scDblFinder parameterizations per sample** and derives **three scDblFinder_prior_sensitivity strategies** for downstream filtering. All methods use `clusters = FALSE` (random approach), as benchmarking reports similar performance regardless of cluster structure ([Germain et al., 2021](https://f1000research.com/articles/10-979)).
...
3. `expected_dbr_` `r expected_dbr_value` (`scDblFinder.class.expected_dbr`): higher fixed expected doublet rate as specified in the `yaml`. The user can specify the value to use here, e.g. based on the anticipated proportion of doublets in the dataset. 

Three `scDblFinder_prior_sensitivity` strategies are also calculated:

4. `scDblFinder_prior_sensitivity_any` (`scDblFinder.class.scDblFinder_prior_sensitivity_any`): a cell is classified as a doublet if **at least one** of the three methods calls it a doublet (most permissive).
5. `scDblFinder_prior_sensitivity_majority` (`scDblFinder.class.scDblFinder_prior_sensitivity_majority`): a cell is classified as a doublet if **at least two** of the three methods call it a doublet.
6. `scDblFinder_prior_sensitivity_all` (`scDblFinder.class.scDblFinder_prior_sensitivity_all`): a cell is classified as a doublet only if **all three** methods call it a doublet (most stringent).
...
# Exclude plots under any */sublibrary/* directory
seurat_plots_file <- seurat_plots_file[!grepl("/sublibrary/", seurat_plots_file)]
...
We will summarize the estimated and removed doublets per sample and per method. Because expected doublet rates for Parse Biosciences libraries may vary by sample type, loading strategy, and tissue characteristics, three scDblFinder parameterizations (`no_dbr_dbr.sd_1`, `low_dbr_0.0003`, `expected_dbr_` `r expected_dbr_value`) and three `scDblFinder_prior_sensitivity` strategies (`scDblFinder_prior_sensitivity_any`, `scDblFinder_prior_sensitivity_majority`, `scDblFinder_prior_sensitivity_all`) were evaluated per sample. Results are shown by sample (as ordered in the `Samples` section). This is an optional step, so it will be skipped or not as defined in `params`.
...
Doublet prediction UMAP plots are generated for each sample and each evaluated method (`no_dbr_dbr.sd_1`, `low_dbr_0.0003`, `expected_dbr_` `r expected_dbr_value`, and `scDblFinder_prior_sensitivity` strategies).
...
[scDblFinder](https://bioconductor.org/packages/devel/bioc/vignettes/scDblFinder/inst/doc/scDblFinder.html) is an R package designed for identifying doublets in single-cell RNA-seq data. For Parse Biosciences data, this pipeline evaluates multiple parameterizations (`dbr.sd = 1`, `dbr = 0.0003`, `dbr = ` `r expected_dbr_value`) and `scDblFinder_prior_sensitivity` strategies to account for uncertain platform-specific doublet rates.
