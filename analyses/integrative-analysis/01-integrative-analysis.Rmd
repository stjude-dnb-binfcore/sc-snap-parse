---
title: "Integrative analysis for sc-/sn-RNA-Seq Analysis in PARSE BIOSCIENCES sequencing technology data"
...
if (use_seurat_integration == "YES"){
  ...
  seurat_obj_list <- SplitObject(merged_obj, split.by = "ID")
...
if (use_harmony_integration == "YES"){
  ...
  for(current_var in variable_value){
    print(glue::glue("Plotting {current_var}"))
    # Plot
    name <- paste0(plots_dir, "/", glue::glue("plot_{integration_method}_{current_var}.png"))
    p1 <- DimPlot(object = seurat_obj, reduction = glue::glue("{integration_method}"), pt.size = .1, group.by = current_var) + NoLegend()
    p2 <- VlnPlot(object = seurat_obj, features = "harmony_1", group.by = current_var, pt.size = .1) + NoLegend()
...
if (use_liger_integration == "YES"){
  ...
  for(current_var in variable_value){
    ...
    # Plot
    name <- paste0(plots_dir, "/", glue::glue("plot_{integration_method}_{current_var}.png"))
    DimPlot(seurat_obj, reduction = glue::glue("{integration_method}"), group.by = current_var) + ggtitle(glue::glue("Plot {integration_method}: {current_var}"))
...
for(current_var in variable_value){
  print(glue::glue("Plotting {current_var}"))
  name <- paste0(plots_dir, "/", glue::glue("plot_unitegrated_vs_{integration_method}_{current_var}.png"))
  p1 <- DimPlot(merged_obj, group.by = current_var) + ggtitle("Before integration")
  p2 <- DimPlot(seurat_obj, reduction = "umap", group.by = current_var) + ggtitle(glue::glue("After {integration_method} integration: {current_var}"))  
  print(patchwork::wrap_plots(list(p1, p2), nrow = 1))
  ggsave(file = name, width = 12, height = 5, device = "png")
}
```
...
for(current_var in variable_value){
  print(glue::glue("Plotting {current_var}"))
  n_groups <- length(unique(seurat_obj@meta.data[[current_var]]))
  colors_current <-scales::hue_pal()(n_groups)
  
  # Plot
  name <- paste0(plots_dir, "/", glue::glue("plot_fraction_of_cells_{integration_method}_{current_var}.png"))
  print(plot_integrated_clusters(seurat_obj, palette = colors_current, variable_value = current_var))
